...
alter table sessions
  ...
  add column if not exists partner_weight_min integer,
  add column if not exists partner_weight_max integer,
  add column if not exists partner_height_min integer,
  add column if not exists partner_height_max integer,
  add column if not exists partner_dominant_hand text,
  add column if not exists partner_glove_size text;
...
select
  ...
  p.lat as place_lat,
  p.lng as place_lng,
  s.partner_weight_min,
  s.partner_weight_max,
  s.partner_height_min,
  s.partner_height_max,
  s.partner_dominant_hand,
  s.partner_glove_size,
...
group by s.id, t.name, p.name, p.city, p.lat, p.lng, sb.ends_at, pr.display_name, pr.email;
